...
export class WhatsAppController {
  ...
  async configureWebhook(@Param("id") id: string, @Body() dto: { url: string }) {
    return this.whatsappService.configureWebhook(id, dto.url);
  }
  ...
  async getHealth(@Param("id") id: string) {
    return this.whatsappService.getHealth(id);
  }

  @Post("connections/:id/send-message")
  @UseGuards(OrganizationGuard, PermissionGuard)
  @RequirePermission("message.send")
  async sendMessage(@Param("id") id: string, @Body() dto: { to: string; text: string }) {
    return this.whatsappService.sendMessage(id, dto.to, dto.text);
  }

  @Post("connections/:id/send-template")
  @UseGuards(OrganizationGuard, PermissionGuard)
  @RequirePermission("message.send")
  async sendTemplate(@Param("id") id: string, @Body() dto: { to: string; templateName: string; parameters: any[] }) {
    return this.whatsappService.sendTemplate(id, dto.to, dto.templateName, dto.parameters);
  }

  @Post("connections/:id/broadcast")
  @UseGuards(OrganizationGuard, PermissionGuard)
  @RequirePermission("message.broadcast")
  async broadcast(@Param("id") id: string, @Body() dto: { contacts: string[]; text: string }) {
    return this.whatsappService.broadcast(id, dto.contacts, dto.text);
  }

  @Get("media/:messageId")
  @UseGuards(OrganizationGuard, PermissionGuard)
  @RequirePermission("message.read")
  async downloadMedia(@Query("connectionId") connectionId: string, @Param("messageId") messageId: string) {
    return this.whatsappService.getMedia(connectionId, messageId);
  }
}

...
export default function MarketingSettingsPage() {
  ...
  if (!s) return <div className="p-8 text-muted">تعذر تحميل الإعدادات.</div>;

  return (
    <div className="p-8 max-w-2xl space-y-6">
      <div className={`border rounded-lg p-5 shadow-sm ${s.killSwitchEnabled ? "bg-danger-50 dark:bg-danger-600/10 border-danger-500/30" : "bg-surface"}`}>
        <div className="flex items-start justify-between">
          <div>
            <h3 className="font-bold text-content flex items-center gap-2">
              <AlertTriangle className={`w-5 h-5 ${s.killSwitchEnabled ? "text-danger-600 dark:text-danger-400" : "text-faint"}`} />
              إيقاف الإرسال التسويقي بالكامل
            </h3>
            <p className="text-sm text-muted mt-1">يوقف كل الرسائل التسويقية فوراً. لا يؤثر على دعم العملاء العادي عبر واتساب.</p>
          </div>
          <button onClick={() => save({ killSwitchEnabled: !s.killSwitchEnabled })} disabled={saving}
            className={`relative w-12 h-6 rounded-full transition ${s.killSwitchEnabled ? "bg-danger-600" : "bg-ink-200 dark:bg-ink-700"}`}>
            <span className={`absolute top-0.5 w-5 h-5 bg-surface rounded-full transition ${s.killSwitchEnabled ? "right-0.5" : "left-0.5"}`} />
          </button>
        </div>
      </div>
      ...
      <SettingsCard title="حدود الحملات والتكلفة" desc="ضوابط الأمان ومنع الإنفاق المفرط">
        <div className="flex items-center justify-between py-2">
          <div>
            <p className="text-sm font-medium text-content">الوضع الآمن (Canary)</p>
            <p className="text-xs text-muted">يحدّ عدد المستقبلين في كل حملة لأول تجارب مضبوطة</p>
          </div>
          <button onClick={() => save({ safeCampaignMode: !s.safeCampaignMode })} disabled={saving}
            className={`relative w-12 h-6 rounded-full transition ${s.safeCampaignMode ? "bg-qano-600" : "bg-ink-200 dark:bg-ink-700"}`}>
            <span className={`absolute top-0.5 w-5 h-5 bg-surface rounded-full transition ${s.safeCampaignMode ? "right-0.5" : "left-0.5"}`} />
          </button>
        </div>
      ...
      <div className="text-xs text-faint">الحد الأقصى للخصم على مستوى المنصة: 5% (ثابت، غير قابل للتجاوز).</div>
      {saved && <div className="flex items-center gap-2 text-qano-600 dark:text-qano-400 text-sm"><CheckCircle className="w-4 h-4" /> تم الحفظ</div>}
    </div>
  );
}

  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="bg-white border rounded-lg p-5 shadow-sm mb-4">
        ...
        {error && <div className="bg-red-50 text-red-700 text-sm p-2 rounded mt-3">{error}</div>}
      </div>
      ...
      <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center text-gray-500 flex flex-col items-center">
            ...
            <p className="font-medium text-gray-900">القائمة فارغة</p>
          </div>
        ) : (
          <table className="w-full text-sm text-right">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-6 py-3 font-medium text-gray-500">الرقم</th>
                <th className="px-6 py-3 font-medium text-gray-500">المصدر</th>
                <th className="px-6 py-3 font-medium text-gray-500">السبب</th>
            ...
            <tbody className="divide-y">
              {data.items.map((e: any) => (
                <tr key={e.id} className="hover:bg-gray-50/50">
                  <td className="px-6 py-3 text-gray-700" dir="ltr">{e.normalizedPhone}</td>
                  <td className="px-6 py-3 text-gray-400 text-xs">{e.source}</td>
                  <td className="px-6 py-3 text-gray-500">{e.reason || "—"}</td>

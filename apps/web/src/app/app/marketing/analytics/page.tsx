...
export default function MarketingAnalyticsPage() {
  ...
  if (!data) return <div className="p-8 text-muted">لا توجد بيانات.</div>;
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      {!data.deliveryMetrics?.deliveredAvailable && (
        <p className="text-xs text-faint">حالة التسليم/القراءة: {data.deliveryMetrics?.note || "غير متاح من مزود الاتصال الحالي"}</p>
      )}
    </div>
  );
}
...
function Big({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-surface border rounded-lg p-5 shadow-sm text-center">
...
function Panel({ title, rows, labels }: { title: string; rows?: Record<string, number>; labels: Record<string, string> }) {
  ...
  return (
    <div className="bg-surface border rounded-lg p-5 shadow-sm">
      <h3 className="font-bold text-content mb-3">{title}</h3>
      {entries.length === 0 ? (
        <p className="text-sm text-faint">لا توجد بيانات بعد.</p>
      ) : (

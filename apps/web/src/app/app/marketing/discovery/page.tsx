...
export default function DiscoveryPage() {
  ...
  return (
    <div className="p-8 max-w-2xl">
      {providerStatus && (
        <div className="flex items-center gap-2 mb-4 text-sm bg-qano-50 dark:bg-qano-900 text-qano-700 dark:text-qano-300 p-3 rounded-lg">
      ...
      <div className="bg-surface border rounded-lg p-6 shadow-sm space-y-4">
        <div>
          <label className="block text-sm font-medium text-muted mb-1">البرنامج</label>
        ...
        <div>
          <label className="block text-sm font-medium text-muted mb-1">النشاط</label>
          <Input placeholder="مثال: مكاتب محاماة" value={form.businessType} onChange={(e) => setForm({ ...form, businessType: e.target.value })} />
        </div>
        <div>
          <label className="block text-sm font-medium text-muted mb-1">المدينة</label>
          <Input placeholder="مثال: الرياض" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} />
        </div>
        <div>
          <label className="block text-sm font-medium text-muted mb-1">العدد المطلوب</label>
      ...
      {result && (
        <div className="bg-surface border rounded-lg p-6 shadow-sm mt-4">
          <h3 className="font-bold text-content mb-3">نتيجة الاكتشاف</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-center">
            <Stat label="تم العثور" value={result.found} />
            <Stat label="أُضيف" value={result.created} color="text-qano-600 dark:text-qano-400" />
            <Stat label="مكرر" value={result.duplicates} />
            <Stat label="عدم تواصل" value={result.dnc} color="text-danger-600 dark:text-danger-400" />
            <Stat label="رقم غير صالح" value={result.invalidPhone} />
          </div>
          <p className="text-xs text-faint mt-3">المزوّد: {result.provider} — الحالة: {statusLabel[result.providerStatus] || result.providerStatus}</p>
        </div>
      )}
    </div>
  );

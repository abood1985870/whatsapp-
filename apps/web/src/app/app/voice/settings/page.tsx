  ...
  return (
    <button onClick={onClick} disabled={disabled} aria-pressed={on}
      className={`relative w-12 h-6 rounded-full transition shrink-0 disabled:opacity-50 ${
        on ? (danger ? "bg-danger-600" : "bg-qano-600") : "bg-ink-200 dark:bg-ink-700"
      }`}>
      <span className={`absolute top-0.5 w-5 h-5 bg-surface rounded-full transition ${on ? "right-0.5" : "left-0.5"}`} />
    </button>
  );
}
